...
export const confirmReservation = async (req, res) => {
    ...
    if (!reservation.serviceid && reservation.packid){
      const conflictingReservation = await prisma.reservations.findFirst({
        where: {
          Pack: {
            prestataireid: reservation.Pack.prestataireid,
          },
          dateDebut: reservation.dateDebut,
          Status: "CONFIRMED",
          id: { not: reservationId },
        ...
        html: `
          <div style="font-family: 'Montserrat', 'Arial', sans-serif; background-color: #f8f9fa; padding: 40px 0; margin: 0; color: #333;">
            <div style="max-width: 650px; margin: 0 auto; background-color: #ffffff; border-radius: 16px; box-shadow: 0 10px 25px rgba(0,0,0,0.05); overflow: hidden;">
              ...
              <div style="width: 100%; height: 200px; overflow: hidden; position: relative;">
                <div style="position: absolute; bottom: 0; left: 0; right: 0; background: linear-gradient(transparent, rgba(0,0,0,0.7)); padding: 20px 15px 15px; color: white;">
                  <h2 style="margin: 0; font-size: 22px;">${reservation.Pack.title}</h2>
                  <div style="display: flex; align-items: center; margin-top: 5px;">
                    <img src="${iconStarUrl}" alt="Étoile" style="width: 16px; height: 16px; margin-right: 5px;">
                  </div>
                </div>
              ...
              <div style="padding: 30px;">
                ...
                <div>
                  ...
                  <p style="font-size: 16px; line-height: 1.6; margin: 0;">
                    Nous sommes ravis de vous confirmer que votre réservation a été acceptée par <strong>${reservation.Pack.Prestataire.prenom} ${reservation.Pack.Prestataire.nom}</strong>. Tous les détails sont prêts et nous sommes impatients de contribuer à la réussite de votre événement.
                  </p>
                </div>
                ...
                <div style="background-color: #f9f9f9; border-radius: 12px; padding: 25px; margin-bottom: 30px; box-shadow: 0 2px 8px rgba(0,0,0,0.04);">
                  ...
                  <div style="display: flex; margin-bottom: 15px; align-items: flex-start;">
                    <img src="${iconCalendarUrl}" alt="Calendrier" style="width: 20px; height: 20px; margin-right: 15px; margin-top: 2px; flex-shrink: 0;">
                    <div>
                      <p style="margin: 0 0 5px 0; font-weight: 600;">Période de réservation</p>
                      <p style="margin: 0; color: #555;">${formatDate(reservation.dateDebut)}</p>
                    </div>
                  </div>
                  ...
                  <div style="display: flex; margin-bottom: 15px; align-items: flex-start;">
                    ...
                    <div>
                      <p style="margin: 0 0 5px 0; font-weight: 600;">Prestataire</p>
                      <p style="margin: 0; color: #555;">${reservation.Pack.Prestataire.prenom} ${reservation.Pack.Prestataire.nom}</p>
                    </div>
                  </div>  
                  ${
                    reservation.prix
                ...
                <!-- QR Code et actions -->
                 <div style="display: flex; justify-content: center; gap: 30px; flex-wrap: wrap; margin-top: 30px;">
                <a href="https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(`Réservation: ${reservation.Pack.title}`)}&dates=${new Date(reservation.dateDebut).toISOString().replace(/-|:|\.\d+/g, "")}/${new Date(reservation.dateFin || new Date(new Date(reservation.dateDebut).getTime() + 3600000)).toISOString().replace(/-|:|\.\d+/g, "")}&details=${encodeURIComponent(`Réservation confirmée pour le pack ${reservation.Pack.title} avec ${reservation.Pack.Prestataire.prenom} ${reservation.Pack.Prestataire.nom}`)}" 
                  target="_blank" 
                  style="background-color: #4CAF50; color: white; text-decoration: none; padding: 12px 20px; border-radius: 8px; text-align: center; font-weight: 600; font-size: 15px; min-width: 140px;">
                  Payer
                </a>
                &nbsp; &nbsp; &nbsp; 
                <a href="mailto:${reservation.Pack.Prestataire.email || "contact@fleskevent.com"}?subject=${encodeURIComponent(`Question sur ma réservation: ${reservation.Pack.title}`)}" 
                  style="background-color: #f8f9fa; border: 1px solid #ddd; color: #333; text-decoration: none; padding: 12px 20px; border-radius: 8px; text-align: center; font-weight: 600; font-size: 15px; min-width: 140px;">
                  Contacter le prestataire
                </a>
              </div>
...
export const getAllReservationServicesOnly = async (req, res) => {
  try {
    ...
    if (reservations.length === 0) {
      return res.status(404).json({ message: "Aucune réservation de service trouvée." });
    }
...
export const getAllReservationPacksOnly = async (req, res) => {
  try {
    const reservations = await prisma.reservations.findMany({
      where: {
        packid: {
    ...
    if (reservations.length === 0) {
      return res.status(404).json({ message: "Aucune réservation de pack trouvée." });
    }

    return res.status(200).json({ reservations });
  } catch (error) {
    console.error("Erreur lors de la récupération des réservations de packs :", error);
    return res.status(500).json({
      status: 500,
      message: "Une erreur est survenue. Veuillez réessayer.",
    });
...
+              <div style="position: absolute; bottom: 0; left: 0; right: 0; background: linear-gradient(transparent, rgba(0,0,0,0.7)); padding: 20px 15px 15px; color: white;">
            ...
            <div style="padding: 30px;">
              ...
              <div style="margin-bottom: 25px;">
                ...
                <p style="font-size: 16px; line-height: 1.6; margin: 0;">
                  Nous vous confirmons que votre réservation avec <strong style="text-transform: capitalize;">${reservation.Pack.Prestataire.prenom} ${reservation.Pack.Prestataire.nom}</strong> a été annulée. Si vous avez des questions n'hésitez pas a lui contacter à : ${reservation.Pack.Prestataire.numTel}, ou par email ce dessous.
              ...
              <div style="background-color: #f9f9f9; border-radius: 12px; padding: 25px; margin-bottom: 30px; box-shadow: 0 2px 8px rgba(0,0,0,0.04);">
                ...
                <div style="display: flex; margin-bottom: 15px; align-items: flex-start;">
                  ...
                  <div>
                    <p style="margin: 0 0 5px 0; font-weight: 600;">Prestataire</p>
                    <p style="margin: 0; color: #555; text-transform: capitalize;">${reservation.Pack.Prestataire.prenom} ${reservation.Pack.Prestataire.nom}</p>
                  </div>
                </div>  
                ${
                  reservation.prix
              ...
              <!-- Actions -->
                 <div style="display: flex; justify-content: center; gap: 30px; flex-wrap: wrap; margin-top: 30px;">
                <a href="http://localhost:4200/packsOR" 
                  target="_blank" 
                  style="background-color: #e53935; color: white; text-decoration: none; padding: 12px 20px; border-radius: 8px; text-align: center; font-weight: 600; font-size: 15px; min-width: 140px;">
                  Découvrir d'autres packs 
                </a>
&nbsp; &nbsp;
                <a href="mailto:${reservation.Pack.Prestataire.email || "contact@fleskevent.com"}?subject=${encodeURIComponent(`Question sur mon annulation: ${reservation.Pack.title}`)}" 
                  style="background-color: #f8f9fa; border: 1px solid #ddd; color: #333; text-decoration: none; padding: 12px 20px; border-radius: 8px; text-align: center; font-weight: 600; font-size: 15px; min-width: 140px;">
                  Contacter le prestataire
                </a>
              </div>
      ...
      where: {
        OR: [
          ...
          },
          {
            serviceid: { not: null },
            Service: {
              Prestataireid: prestataireId // ⚠ ici c’est bien "Prestataireid"
            }
          }
        ]
      },
